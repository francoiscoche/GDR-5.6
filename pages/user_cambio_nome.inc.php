        ...
        if($_POST['op'] == 'new') {
            if(($email == gdrcd_filter_email($_POST['email'])) && ($pass == gdrcd_encript($_POST['new_pass'])) && ($iscriz >= strftime('%Y-%m-%d')) && (empty($_POST['new_name']) === false)) {
                ...
                if(gdrcd_query($result, 'num_rows') > 0) { ?>
                    ...
                    gdrcd_query("UPDATE msggrpuser set nome = CONCAT(nome,'_del'), tpuser='DELETED', dtend=NOW() WHERE nome = '". gdrcd_filter('in', $_SESSION['login'])."'");
					gdrcd_query("UPDATE msg SET nomesender = CONCAT(nomesender,'_del') WHERE nomesender='".gdrcd_filter('in', $_SESSION['login'])."'");
            ...
            if(($_SESSION['permessi'] >= MODERATOR) && (empty($_POST['new_name']) === false)) {
                ...
                <?php } else {
                    if($_SESSION['permessi'] == SUPERUSER) {
                        ...
                        gdrcd_query("UPDATE msggrpuser set nome = CONCAT(nome,'_del'), tpuser='DELETED', dtend=NOW() WHERE nome = '". gdrcd_filter('in', $_POST['account'])."'");
                        gdrcd_query("UPDATE msg SET nomesender = CONCAT(nomesender,'_del') WHERE nomesender= '".gdrcd_filter('in', $_POST['account'])."'");
