  ...
  ?>
    ...
    <div class="page_body">
        <div class="panels_box">
            ...
            $personaggio = gdrcd_query("SELECT stato FROM personaggio WHERE nome = '".gdrcd_filter('in', $_REQUEST['pg'])."'");
             ?>
            <div class="body_box">
                    <?php
                    ...
                    if($PARAMETERS['mode']['user_bbcode'] == 'ON') {
                        if($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd' && $PARAMETERS['settings']['bbd']['free_html'] == 'ON') {
                            echo bbdecoder(gdrcd_html_filter($personaggio['stato']), true);
                        } elseif($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd') {
                            echo bbdecoder(gdrcd_filter('out', $personaggio['stato']), true);
                        } else {
                            echo gdrcd_bbcoder(gdrcd_filter('out', $personaggio['stato']));
                        }
                    } else {
                        echo gdrcd_html_filter($personaggio['stato']);
                    } ?>
            </div>
        </div>
